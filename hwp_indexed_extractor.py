...
# 기존 유틸 함수 재사용
from pdf_parser_odl import normalize_strata_name, natural_sort_key
...
def clean_float(val):
    """숫자 추출: 범위(~), 쉼표, 공백, 단위 등을 제거하고 float로 변환"""
    if val is None:
        return None
    s = clean_cell_text(str(val))
    if not s:
        return None
    # 범위 표기(~) → 마지막 값 사용
    if '~' in s:
...
def normalize_bh_id(raw_id):
    """시추번호 정규화: 중복 하이픈/공백 제거, 대문자 변환"""
    text = clean_cell_text(raw_id)
    if not text:
        return ""
    # 중복 하이픈/공백 → 단일 하이픈
