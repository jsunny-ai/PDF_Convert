...
def clean_float(val_str):
    """숫자 추출 시 범위(~), 쉼표(,), 공백, 단위 등을 제거하고 float로 변환"""
...
def find_metadata_spatial(page):
    """페이지 내 메타데이터(표고, 좌표)를 공간적으로 추출"""
    # \u00A7 = ǥ (표고 깨짐 대응)
    el = find_value_by_spatial_proximity(page, ["표고", "E.L", "EL", "ǥ", "\u00A7", "EL."])
    # 기존 일관성을 위해 X(N)을 경도, Y(E)를 위도로 매핑 (PJ_249-2 기준)
...
def extract_all_from_md(md_path: str, project_name: str = "", pdf_path: str = None):
    ...
    ERROR_LOG_PATH = r"C:\antigravity\#1_2_PDF_CSV\error_log.txt"
    def _log_error(msg):
...
def extract_from_pdf_text_odl(pdf_path: str, project_name: str = ""):
    ...
    md_path = os.path.join(r"C:\antigravity\#1_2_PDF_CSV\MD_Storage", f"{base_name}.md")
