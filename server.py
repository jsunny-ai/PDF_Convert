...
import table_merger
from master_hybrid_extractor import MasterHybridExtractor, get_csv_headers

app = Flask(__name__, static_folder='.', static_url_path='')

UPLOAD_FOLDER = os.path.join(app.root_path, 'data', '01_raw_pdf', 'temp_uploads')
...
def convert_pdf():
    ...
    for file in uploaded_files:
        ...
        try:
            ...
            # 물리적 파일명은 ASCII로 유지 (download_file에서 seoul_borehole_master.csv로 변환됨)
            ...
            if merged:
                ...
                # 컬럼 순서 고정
                column_order = ['프로젝트명', '경도', '위도', '표고', '시추공명', '상심도', '하심도', '지층명']
...
def download_file(request_id, filename):
    ...
    if is_csv:
        ...
        # 개별 파일 다운로드 요청인 경우 (URL에 구체적인 힌트가 있는 경우) 대응 로직 추가 가능하나
        # 현재는 통합 마스터 다운로드를 우선시함.
    else:
        physical_name = "all_results_archive.zip"

    full_path = os.path.join(app.config['UPLOAD_FOLDER'], request_id, physical_name)
    
    # 만약 위 규칙으로 못찾는다면 (개별 CSV 다운로드 등), filepath 직접 매핑 시도
    if not os.path.exists(full_path) and is_csv:
        # 개별 파일의 경우 request_id/individual/ 폴더 내에 존재할 수 있음
    ...
    # 3. 보정된 파일명 결정 (사용자 요청: seoul_borehole_master.csv)
    serve_name = "seoul_borehole_master.csv" if is_csv else "all_results_archive.zip"
